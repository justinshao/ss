...
public static class ByteArrayExtensions
{
    ...
    private static decimal toDecimal(byte[] data, int offset, int count, int width)
    {
        if (width < 0 || width > 16) { throw new ArgumentOutOfRangeException("width", "width must between 1 and 16"); }
    ...
    /// <summary>
    /// 将 byte[] 数组转换为 decimal (最多 16 byte)
    /// </summary>
    /// <param name="data">要转换为 decimal 类型的 byte 数组</param>
    /// <param name="offset">转换的起始位置</param>
    /// <param name="count">转换的 byte 数量，最多16 byte</param>
    /// <returns></returns>
    public static decimal ToDecimal(this byte[] data, int offset, int count)
    {
        checkLength(data);
        return toDecimal(data, offset, count, 16);
    }
    public static decimal ToDecimal(this byte[] data, int offset)
    {
        checkLength(data);
        return toDecimal(data, offset, 16, 16);
    }
    public static decimal ToDecimal(this byte[] data)
    {
        checkLength(data);
        return toDecimal(data, 0, 16, 16);
    }
    ...
    {
        checkLength(data);
        return (short)toLong(data, offset, 2, 2);
    }
    public static short ToShort(this byte[] data, int offset, int count)
    {
        checkLength(data);
        return (short)toLong(data, offset, count, 2);
    }
    ...
    {
        checkLength(data);
        return (ushort)toLong(data, offset, 2, 2);
    }
    public static ushort ToUShort(this byte[] data, int offset, int count)
    {
        checkLength(data);
        return (ushort)toLong(data, offset, count, 2);
    }
    ...
    {
        checkLength(data);
        return (int)toLong(data, offset, 4, 4);
    }
    public static int ToInt(this byte[] data, int offset, int count)
    {
        checkLength(data);
        return (int)toLong(data, offset, count, 4);
    }
...
public static class ShortExtensions
{
    public static byte[] ToBytes(this short num)
    {
        ...
        {
            result[i] = (byte)(num >> 8 * (length - 1 - i));
        }
        return result;
    }
}
...
public static class IntExtensions
{
    public static byte[] ToBytes(this int num)
    {
        int length = 4;
        byte[] result = new byte[length];
        num.ToBytes(result, 0);
        return result;
    }

    public static void ToBytes(this int num, byte[] bytes, int offset)
    {
        int length = 4;
        for (int i = 0; i < length; i++)
        {
            bytes[offset + i] = (byte)(num >> 8 * (length - 1 - i));
        }
    }

    public static void GetBytes(this int num, byte[] bytes, int offset)
    {
        bytes[offset] = (byte)(num & 0x000000ff);
        bytes[offset + 1] = (byte)((num & 0x0000ff00) >> 8);
        bytes[offset + 2] = (byte)((num & 0x00ff0000) >> 16);
    ...
    public static double ToYuan(this int fen)
    {
        return (fen) / 100;
    }
    public static double ToYuan(this decimal fen)
    {
        return (double)(fen) / 100;
    }
    private static string[] strMonth = new string[] { "", "一个月", "二个月", "三个月", "四个月", "五个月", "六个月", "七个月", "八个月", "九个月", "十个月", "十一个月", "十二个月" };
    public static string ToMonthDes(this int month)
    {
        if (month < 1 || month > 12)
            return string.Empty;
...
public static class DateTimeExtensions
{
    ...
    /// </summary>
    /// <param name="dt"></param>
    /// <returns>eg: 09NOV</returns>
    public static string GetNamedDate(this DateTime dt)
    {
        return dt.ToString("ddMMM", CultureInfo.InvariantCulture).ToUpper();
    }

    /// <summary>
    /// 获取带有年份的命名日期 eg: 09NOV10
    /// </summary>
    /// <param name="dt"></param>
    /// <returns></returns>
    public static string GetNamedDateWithYear(this DateTime dt)
    {
        return dt.ToString("ddMMMyy", CultureInfo.InvariantCulture).ToUpper();
    }

    public static string GetNamedMonthWithYear(this DateTime dt)
    {
        return dt.ToString("MMMyy", CultureInfo.InvariantCulture).ToUpper();
    }
    /// <summary>
    /// 获取精简时间 eg: 1300
    /// </summary>
    /// <param name="dt"></param>
    /// <returns>eg: 1300</returns>
    public static string GetSimpleTime(this DateTime dt)
    {
        return dt.ToString("HHmm", CultureInfo.InvariantCulture);
    }
    ...
    public static bool Between(this DateTime value, string from, string to, string format)
    {
        string currentTime = value.ToString(format);
        return string.Compare(from, currentTime) < 1 && string.Compare(currentTime, to) < 1;
    }

    public static int CompareTo(this DateTime value, DateTime time, string format)
    {
        return string.Compare(value.ToString(format), time.ToString(format));
    }
    ...
    /// <param name="pnrTime">13:30</param>
    /// <returns></returns>
    public static DateTime Add(this DateTime date, string pnrTime)
    {
        return DateTime.Parse(date.ToString("yyyy-MM-dd " + pnrTime));
    }
    ...
    /// <returns>通用的yyyy-MM-dd格式</returns>
    public static string ToPlatString(this DateTime value)
    {
    ...
    /// </summary>
    /// <param name="value">时间</param>
    /// <returns>通用的yyyy年MM月dd日 格式</returns>
    public static string ToPlatString(this DateTime? value)
    {
        return value.HasValue ? value.Value.ToString("yyyy年MM月dd日") : "";
    }

    /// <summary>
    /// 显示平台通用的yyyy年MM月dd日 HH时mm分 格式
    /// </summary>
    /// <param name="value">时间</param>
    /// <returns>通用的yyyy年MM月dd日 HH时mm分 格式</returns>
    public static string ToPlatDateTimeString(this DateTime value)
    {
        return value.ToString("yyyy年MM月dd日 HH时mm分");
    }
    ...
    /// </summary>
    /// <param name="value">时间</param>
    /// <returns>通用的yyyy年MM月dd日 HH时mm分格式</returns>
    public static string ToPlatDateTimeString(this DateTime? value)
    {
        return value.HasValue ? value.Value.ToString("yyyy年MM月dd日 HH时mm分") : "";
    }
    ...
    /// <returns></returns>
    public static string GetSimpleFormat(this DateTime value)
    {
    ...
    /// <param name="value"></param>
    /// <returns></returns>
    public static string GetTotalFormat(this DateTime value)
    {
        return value.ToString("yyyyMMdd");
    }
    /// <summary>
    ...
    /// <param name="value"></param>
    /// <returns></returns>
    public static string GetTimeFormat(this DateTime value)
    {
    ...
    /// <param name="value"></param>
    /// <returns></returns>
    public static string GetFlightDate(this DateTime value)
    {
        return value.ToString("ddMMM(ddd)", CultureInfo.InvariantCulture).ToUpper();
    }
    /// <summary>
    /// 获取停车时长
    /// </summary>
    /// <param name="starttime">开始时长</param>
    /// <param name="endtime">结束时长</param>
    /// <returns></returns>
    public static string GetParkingDuration(this DateTime starttime, DateTime endtime)
    {
        ...
        if (day > 0)
        {
            return string.Format("{0}天{1}小时{2}分", day, hour, minute);
        }
        if (hour > 0)
        {
    ...
    public static string GetParkingDuration(this int totalMinuteNum)
    {
        int day = (int)(totalMinuteNum / (60 * 24));
        int hh = (int)((totalMinuteNum - (day * 24 * 60)) / 60);
        int mm = (int)(totalMinuteNum - (day * 24 * 60) - hh * 60);
        if (day > 0)
        {
            return string.Format("{0}天{1}小时{2}分", day, hh, mm);
        }
...
public static class IEnumerableExtensions
{
    public static string Join(this IEnumerable<string> source, string separator)
    {
        ...
        var tmp = source.Aggregate(string.Empty, (x, y) => x + separator + y);
        return tmp == string.Empty ? string.Empty : tmp.Substring(separator.Length);
    }

    public static string Join<T>(this IEnumerable<T> source, string separator, Func<T, string> map)
    {
        if (source == null) return string.Empty;

        var v = from element in source
                select map(element);
    ...
    public static string Join<T>(this IList<T> source, string separator, Func<T, string> map)
    {
        if (source == null || source.Count == 0) return string.Empty;
    ...
    public static string JoinByStringBuilder(this IEnumerable<string> source, string splitor)
    {
        if (source == null || source.Count() == 0) return string.Empty;
        ...
        {
            if (sb.Length != 0) { sb.Append(splitor); }
            sb.Append(item);
        }
    ...
    public static string JoinByStringBuilder<T>(this IEnumerable<T> source, string splitor, Func<T, string> map)
    {
        if (source == null || source.Count() == 0) return string.Empty;
    ...
    public static string Join(this IList<string> source, int from, int to, string splitor)
    {
        if (from > to) throw new ArgumentException("from can not bigger than to!");

        if (source == null) return string.Empty;
        if (from > source.Count || from < 0) return string.Empty;
        if (to > source.Count || to < 0) return string.Empty;
    ...
    public static string JoinByStringBuilder(this IList<string> source, string splitor)
    {
        if (source == null || source.Count == 0) return string.Empty;
    ...
    public static string JoinByStringBuilder<T>(this IList<T> source, string splitor, Func<T, string> map)
    {
        if (source == null || source.Count == 0) return string.Empty;
    ...
    public static List<T> Paging<T>(this IEnumerable<T> source, int pageIndex, int pageSize, out int rowCount)
    {
        int itemIndex = 1;
        var query = (from element in source
                     select new { Index = itemIndex++, Item = element }).ToList();
        ...
        var result = (from item in query
                      where startIndex < item.Index
                         && item.Index <= endIndex
                      select item.Item).ToList();
        return result;
    }
    public static List<T> Pagings<T>(this IEnumerable<T> source, int pageIndex, int pageSize)
    {
    ...
    /// <summary>
    /// 返回第一个匹配项的索引。如果没有匹配项，返回-1
    /// </summary>
    /// <typeparam name="T"></typeparam>
    /// <param name="source"></param>
    /// <param name="selector"></param>
    /// <returns></returns>
    public static int FirstMatch<T>(this IList<T> source, Func<T, bool> predicate)
    {
    ...
    /// <returns></returns>
    public static bool HasRepeated<T>(this IList<T> source, IEqualityComparer<T> comparer)
    {
        return source.Count > source.Distinct(comparer).Count();
    }
    public static V GetValue<K, V>(this Dictionary<K, V> source, K key, V defalut)
    {
        if (source == null) { return defalut; }
        V value;
    ...
    public static V GetValue<K, V>(this Dictionary<K, V> source, K key)
    {
        return source.GetValue(key, default(V));
    }
    public static string GetValue(this Dictionary<string, string> source, string key)
    {
        return source.GetValue(key, string.Empty);
    }
...
public static class DecimalExtensions
{
    ...
    /// <summary>
    /// 取数字的百分比字符串（没有百分号，如果需要百分号请使用P格式化字符）
    /// </summary>
    /// <param name="value">数字</param>
    /// <param name="digit">位数</param>
    /// <returns>字符串</returns>
    public static string Percentage(this decimal value, int digit)
    {
        if (digit < 0)
            throw new ArgumentOutOfRangeException("digit不能为负数");

        return (value * 100).ToString("F" + digit);
    }
...
public static class StringExtensions
{
    ...
    public static TimeSpan ToTimeSpan(this string str)
    {
        ...
        if (string.IsNullOrEmpty(str)) return dateTime;
        if (TimeSpan.TryParse(str.Trim(), out dateTime))
        {
            return dateTime;
        }
        return dateTime;
    ...
    public static DateTime ToDateTime(this string str)
    {
        ...
        try
        {
            ...
            {
                dateTime = DateTime.ParseExact(str, "yyyyMMddHHmmss", System.Globalization.CultureInfo.CurrentCulture);
                return dateTime;
            }
            if (str.Length == "yyyyMMdd".Length)
            {
                dateTime = DateTime.ParseExact(str, "yyyyMMdd", System.Globalization.CultureInfo.CurrentCulture);
                return dateTime;
            }
        }
        catch
        ...
        if (DateTime.TryParse(str.Trim(), out dateTime))
        {
            return dateTime;
        }
        return DateTime.MinValue;
    }
    public static string ToPlateNo(this string str)
    {
    ...
    {
        decimal dec = 0;
        if (decimal.TryParse(str.Trim(), out dec))
        {
            return dec;
        }
        return defaultValue;
    ...
    public static int ToInt(this string str, int valueNull)
    {
        if(str==null)
        {
            return 0;
        ...
        int dec;
        if (int.TryParse(str.Trim(), out dec))
        {
            return dec;
        }
        return valueNull;
    ...
    /// <param name="s">字符串</param>
    /// <returns>结果</returns>
    public static bool IsPositiveInt64(this string s)
    {
        Int64 i = 0;
    ...
    /// <param name="s">字符串</param>
    /// <returns>结果</returns>
    public static bool IsNegativeInteger(this string s)
    {
        int i = 0;
        if (int.TryParse(s, out i))
        {
    ...
    /// <param name="s">字符串</param>
    /// <returns>结果</returns>
    public static bool IsNegativeInt64(this string s)
    {
        Int64 i = 0;
        if (Int64.TryParse(s, out i))
        {
    ...
    /// <param name="s">字符串</param>
    /// <returns>结果</returns>
    public static bool IsNegativeNumber(this string s)
    {
        float f = 0;
        if (float.TryParse(s, out f))
        {
    ...
    {
        bool ret = false;
        if (s.IsInteger())//大于零都为true
        {
            ret = s.ToInt() == 0 ? false : true;
        }
        else
        {
    ...
    public static string Right(this string source, int number)
    {
        if (string.IsNullOrEmpty(source) || source.Length <= number)
            return source;

        return source.Substring(source.Length - number, number);
    }

    public static string RemoveLastMatchPart(this string source, string part)
    {
        if (string.IsNullOrEmpty(source)
            || string.IsNullOrEmpty(part)
            || source.EndsWith(part) == false)
            return source;

        return source.Substring(0, source.Length - part.Length);
    }
    ...
    public static string ToNotNull(this string source)
    {
        return ToNotNull(source, " ");
    }
    public static string ToNotNull(this string source, string defaultValue)
    {
    ...
    /// <returns></returns>
    public static string After(this string source, string target)
    {
        if (string.IsNullOrEmpty(source) || string.IsNullOrEmpty(target)) return source;


        return source.Substring(source.IndexOf(target, System.StringComparison.Ordinal) + target.Length);
    }

    public static string FormatLength(this string str, int len)
    {
        if (string.IsNullOrEmpty(str)) return str;
        if (str.Length <= len) return str;
        return str.Substring(0, len);
    }
    public static string ToFormat(this string str, params object[] obj)
    {
...
{
    public static string GetStringDefaultEmpty(this DbDataReader reader, int index)
    {
        return reader.IsDBNull(index) ? string.Empty : reader.GetString(index);
    }

    public static decimal GetDecimalDefaultZero(this DbDataReader reader, int index)
    {
        return reader.IsDBNull(index) ? 0m : reader.GetDecimal(index);
    }

    public static decimal? GetDecimalNullable(this DbDataReader reader, int index)
    {
        return reader.IsDBNull(index) ? null : new Nullable<decimal>(reader.GetDecimal(index));
    }

    public static DateTime GetDateTimeDefaultMin(this DbDataReader reader, int index)
    {
        return reader.IsDBNull(index) ? DateTime.MinValue : reader.GetDateTime(index);
    }

    public static DateTime? GetDateTimeDefaultNull(this DbDataReader reader, int index)
    {
    ...
    public static int GetInt32DefaultZero(this DbDataReader reader, int index)
    {
        return reader.IsDBNull(index) ? 0 : reader.GetInt32(index);
    }

    public static int GetInt32DefaultOne(this DbDataReader reader, int index)
    {
        return reader.IsDBNull(index) ? 1 : reader.GetInt32(index);
    }
    public static int? GetInt32DefaultNull(this DbDataReader reader, int index)
    {
        return reader.IsDBNull(index) ? new Nullable<int>() : reader.GetInt32(index);
    }

    public static bool GetBool(this DbDataReader reader, int index)
    {
        return reader.GetStringDefaultEmpty(index) == "T";
    }
    public static bool? GetNumberBooleanOrNull(this DbDataReader reader, int index)
    {
        return reader.IsDBNull(index) ? null : new bool?(reader.GetInt32(index) == 1);
    }
    public static bool GetNumberBoolean(this DbDataReader reader, int index)
    {
        return reader.GetInt32(index) == 1;
    }
    public static bool GetBitBoolean(this DbDataReader reader, int index)
    {
...
{
    public static void Join(this StringBuilder sb, IList<string> source, string splitor)
    {
    ...
    public static void Join<T>(this StringBuilder sb, IList<T> source, string splitor, Func<T, string> mapping)
    {
        if (source == null || source.Count == 0) return;

        for (int i = 0; i < source.Count; i++)
        {
            sb.Append(mapping(source[i]));
...
public static class DateTimeExtentsions
{
    ...
    {
        return ToDateString(datetime, "yyyy-MM-dd");
    }
...
public static class ObjectExtensions
{
    public static string ToXml(this object obj, Encoding encoding)
    {
        ...
        using (var memoryStream = new MemoryStream())
        {
            ...
            using (var writer = XmlWriter.Create(memoryStream, xmlWriteSettings))
            {
...
public static class BooleanExtensions
{
    ...
    /// <param name="value"></param>
    /// <returns></returns>
    public static string ToValidityText(this bool value)
    {
        return value ? "启用" : "禁用";
    }
}