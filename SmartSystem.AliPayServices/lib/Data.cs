...
namespace SmartSystem.AliPayServices.lib
{
    ...
    public class WxPayData
    {
        ...
        public string ToXml()
        {
            ...
            foreach (KeyValuePair<string, object> pair in m_values)
            {
                ...
                else//除了string和int类型不能含有其他数据类型
                {
                    ...
                    throw new WxPayException("WxPayData字段数据类型错误!" + pair.Key);
                }
            }
            xml += "</xml>";
            return xml;
        ...
        * @throws WxPayException
        */
        public SortedDictionary<string, object> FromXml(string xml,WX_ApiConfig config)
        {
        ...
        */
        public string MakeSign(WX_ApiConfig config)
        {
            //转url格式
            string str = ToUrl();
            //在string后加入API KEY
            str += "&key=" + config.PartnerKey;
            //MD5加密
            var md5 = MD5.Create();
