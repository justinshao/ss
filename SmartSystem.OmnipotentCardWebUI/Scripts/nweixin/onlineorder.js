...
function BindDataTable() {
    $('#tableOrder').datagrid({
        url: '/nwx/WXOnlineOrder/GetOnlineOrderData',
        singleSelect: true,
        nowrap: false,
        striped: true,
        collapsible: false,
        ...
        onBeforeLoad: function (row, data) {
            ...
            if (toolbar.length == 0) {
                $.post('/nwx/WXOnlineOrder/GetOnlieOrderOperatePurview', function (result) {
...
function BindCombobox() {
    $("#cmbOrderStatus").combobox({
        url: '/nwx/WXOnlineOrder/GetOrderStatus',
        valueField: 'id',
        textField: 'text'
    });
    $("#cmbOrderType").combobox({
        url: '/nwx/WXOnlineOrder/GetOrderType',
        valueField: 'id',
        textField: 'text'
    });
}
function BindParkingCombobox(companyId) {
    $("#cmbParking").combobox({
        url: '/nwx/WXOnlineOrder/GetParkingData?companyId=' + companyId,
        valueField: 'id',
        textField: 'text'
    });
}
function BindExternalParkingCombobox() {
    $("#cmExternalPKID").combobox({
        url: '/nwx/WXOnlineOrder/GetExternalParkingData',
        valueField: 'id',
        textField: 'text'
    });
}
function SynchPaymentResult() {
    ...
    $.messager.confirm('系统提示', '确定要同步支付结果吗?',
            function (r) {
                if (r) {
                    $.post('/nwx/WXOnlineOrder/SynchWeiXinPaymentResult?orderId=' + order.OrderID,
                    function (data) {
                        if (data.result) {
                            $.messager.show({
                                width: 200,
...
function ManualRefund() {
    ...
    $.messager.confirm('系统提示', '确定要退款吗?',
            function (r) {
                if (r) {
                    $.post('/nwx/WXOnlineOrder/ManualRefund?orderId=' + order.OrderID,
                    function (data) {
                        if (data.result) {
                            $.messager.show({
                                width: 200,
...
function Export() {
    ...
    if ($("#tableOrder").datagrid("getData").total == 0) {
        $.messager.alert('系统提示', '当前时间段无数据!', 'error');
        return;
    }
    var companyId = selectRow.id;
    var orderId = $("#txtOrderId").textbox("getValue");
    var start = $("#txtOrderStartTime").datetimebox("getValue");
    var end = $("#txtOrderEndTime").datetimebox("getValue");
    var platebefore = $("#defaultProvinceId").find(".l-btn-text").text();
    ...
    $.ajax({
        type: "post",
        url: '/nwx/WXOnlineOrder/Export_OnlineOrderInfo',
        data: "StartTime=" + start + "&EndTime=" + end + "&OrderId=" + orderId + "&PlateNo=" + plateNo + "&Status=" + status + "&OrderType=" + type + "&CompanyId=" + companyId + "&ParkingId=" + parkingId + "&ExternalPKID=" + externalPKID,
        async: true,
