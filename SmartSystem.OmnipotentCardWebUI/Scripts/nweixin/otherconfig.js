$(function () {
    $('#treeCompany').tree({
        ...
        onSelect: function (record) {
            $('#tableOtherConfigBox').datagrid('load', { companyId: record.id });
        }
    });
...
$(function () {
    $('#tableOtherConfigBox').datagrid({
        url: '/nwx/WXOtherConfig/GetWxOtherConfigData',
        singleSelect: true,
        columns: [[
        ...
        onBeforeLoad: function (row, data) {
            ...
            if (toolbar.length == 0) {
                $.post('/nwx/WXOtherConfig/GetWxOtherConfigOperatePurview', function (result) {
        ...
        buttons: [
            {
                ...
                handler: function () {
                    $.ajax({
                        type: "post",
                        url: '/nwx/WXOtherConfig/AddOrUpdate',
...
function Refresh() {
    ...
    $('#tableOtherConfigBox').datagrid('load', { companyId: selectRow.id });
}
function Format() {
    var config = $('#tableOtherConfigBox').datagrid('getSelected');
    if (config == null) {
