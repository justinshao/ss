...
function GetArticle(groupId) {
    ...
    $.ajax({
        url: '/nwx/WXArticle/GetArticle',
        data: "groupId=" + groupId + "&companyId=" + $("#hiddCompanyID").val(),
        type: 'post',
            ...
            if (data.result) {
                ...
                if (data.data.length == 0) {
                    ...
                    for (var i = 0; i < data.data.length; i++) {
                        ...
                        appmsgmodel.find(".companyid").val(data.data[i].CompanyID);
...
$(function () {
    ...
    //文件上传前触发事件
    $('#fileUpload_ArticleCover').bind('fileuploadsubmit', function (e, data) {
...
function BindComboboxData() {
    $('#cmbArticleType').combobox({
        url: '/nwx/WXArticle/GetArticleTypeData',
    ...
    $('#cmdModule').combobox({
        url: '/nwx/WXArticle/GetWeiXinModuleData',
        valueField: 'EnumValue',
        textField: 'Description',
        onSelect: function (record) {
...
function btnSubmitArticleData() {
    ...
    $(".appmsg_item").each(function () {
        ...
        item.companyid = $("#hiddCompanyID").val();
        item.sort = itemnum;
        jsonData.push(item);
        itemnum++;
    });
    ...
    $.post('/nwx/WXArticle/SaveArticle', pdata, function (data) {
        if (data.result) {
            $.messager.alert('系统提示', "保存成功", 'success');
            location.href = "/nwx/WXArticle/Index";
        } else {
            $.messager.alert('系统提示', data.msg, 'error');
